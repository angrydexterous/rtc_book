...
TEST_F(CanvasTest, canvas_creation_size_and_starting_colors)
{
  auto runTest = []<typename T>(T black)
  {
    ...
    auto color = Color::Color(black, black, black);
    bool allGood = true;
    for (auto j = 0; j < h && allGood; j++)
    {
      for (auto i = 0; i < w; i++)
      {
        if (c.pixelAt(i, j) != color)
        {
          allGood = false;
          break;
        }
...
TEST_F(CanvasTest, canvas_creation_set_color)
{
  ...
  auto color = Color::Color(1.0f, 0.0f, 0.0f);
...
TEST_F(CanvasTest, canvas_construct_ppm_payload)
{
  ...
  constexpr int h = 3;
  auto canvas = Canvas<float>(w, h);
  canvas.writePixel(Color::Color(1.5f, 0.f, 0.f), 0, 0);
  canvas.writePixel(Color::Color(0.f, 0.5f, 0.f), 2, 1);
  canvas.writePixel(Color::Color(-0.5f, 0.f, 1.f), 4, 2);
